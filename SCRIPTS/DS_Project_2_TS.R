...
acc.acf <- ggAcf(e.ts.accidents)
acc.pacf <- ggPacf(e.ts.accidents)
...
# Extract Year and Month
aviation_data$YearMonth <- format(aviation_data$EventDate, "%Y-%m")  # Creates "YYYY-MM" format

# Aggregate fatalities per month
fatal_accidents_monthly <- aviation_data %>%
  group_by(YearMonth) %>%
  summarise(Fatalities = sum(FatalInjuryCount, na.rm = TRUE)) %>%
  arrange(YearMonth)

# Convert YearMonth to Date format
fatal_accidents_monthly$YearMonth <- as.Date(paste0(fatal_accidents_monthly$YearMonth, "-01"))

# Convert to time series object
fatal_ts <- ts(fatal_accidents_monthly$Fatalities, start = c(2015, 1), frequency = 12)
...
# ACF and PACF plots
acf_plot <- ggAcf(fatal_ts) + ggtitle("ACF of Fatal Accidents")
pacf_plot <- ggPacf(fatal_ts) + ggtitle("PACF of Fatal Accidents")

# Combine plots
ggarrange(acf_plot, pacf_plot, nrow = 2, ncol = 1)


###Residuals
residuals_fatal <- residuals(accidents.trend.seasonal.monthly)

# Convert residuals to time series
residuals_ts <- ts(residuals_fatal, start = c(2015, 1), frequency = 12)

#check for patterns in residuals
autoplot(residuals_ts) +
  ggtitle("Residuals of Fatal Accidents Model") +
  xlab("Year") + 
  ylab("Residuals") +
  theme_minimal()

acf_resid_plot <- ggAcf(residuals_ts) + ggtitle("ACF of Model Residuals")
pacf_resid_plot <- ggPacf(residuals_ts) + ggtitle("PACF of Model Residuals")

# Combine plots
ggarrange(acf_resid_plot, pacf_resid_plot, nrow = 2, ncol = 1)


Box.test(residuals_ts, lag = 12, type = "Ljung-Box")
